...
- **Entrée en jeu** : depuis `MainMenu` (Jouer → Multijoueur → choix du deck → Lancer), aucune scène séparée n'est ouverte — `MainMenu._on_launch_pressed` appelle `MatchmakingOverlay.open_mode_picker()`, qui affiche un popup par-dessus le menu avec trois modes — Normal (matchmaking Steam automatique : cherche un lobby existant puis héberge en secours si aucun n'est trouvé, sans choix héberger/rejoindre laissé au joueur), Classé (file d'attente backend, voir « Ranked / paliers » ci-dessous) et Contre un ami (hébergement + overlay Steam d'invitation). `MatchmakingOverlay` (autoload, `scripts/net/MatchmakingOverlay.gd` + scène `scenes/net/MatchmakingOverlay.tscn`, un `CanvasLayer` donc jamais affecté par un `change_scene_to_file`) porte toute la logique de matchmaking : dès qu'un mode est choisi, le popup se ferme et un bandeau de recherche/statut s'affiche en haut à droite, **par-dessus n'importe quelle scène** — le joueur peut continuer à naviguer (deck builder, boutique...) pendant la recherche, et l'annuler depuis le bandeau (bouton ✕) à tout moment. Une fois un adversaire trouvé, un écran de chargement plein écran (fond du menu principal, cercle animé, astuces de jeu défilantes) s'affiche le temps du handshake `NetHandshake` (échange des decks, graine RNG partagée, premier joueur) puis de la synchronisation `NetBattleSync` ; une fois les deux clients synchronisés, une brève présentation face-à-face (nom Steam + race du deck de chaque camp, voir `MatchmakingOverlay._show_vs_screen`) s'affiche avant que les deux clients ne basculent sur `Battle.tscn` ensemble ; `NetContext` (statique) transporte le `NetworkManager` et le résultat du handshake à travers le changement de scène.
