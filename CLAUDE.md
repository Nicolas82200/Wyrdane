...
- **Indicateurs non basés sur la seule couleur** : symbole de rareté sur le bandeau de type d'une carte (`Card.RARITY_SYMBOLS` : ●/◆/▲/★, en plus de `RARITY_COLORS`), icône « ⚠ » devant les HP du héros sous 30 % de ses HP max (`HeroSystem._hp_label_text`/`LOW_HP_RATIO`). Les mots-clés (`TooltipData.KEYWORD_ICONS` et variantes par race) étaient déjà iconifiés individuellement, pas seulement colorés.
