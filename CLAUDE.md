...
- **Entrée en jeu** : `scenes/net/NetLobby.tscn` propose trois modes — Normal (matchmaking Steam automatique : cherche un lobby existant puis héberge en secours si aucun n'est trouvé, sans choix héberger/rejoindre laissé au joueur), Classé (file d'attente backend, voir « Ranked / paliers » ci-dessous) et Contre un ami (hébergement + overlay Steam d'invitation) — aucun bouton "héberger" séparé. Pendant la recherche, un bandeau s'affiche en haut à droite de l'écran ; une fois un adversaire trouvé, un écran de chargement plein écran (fond du menu principal, cercle animé, astuces de jeu défilantes) s'affiche le temps du handshake `NetHandshake` (échange des decks, graine RNG partagée, premier joueur) puis de la synchronisation `NetBattleSync` ; une fois les deux clients synchronisés, une brève présentation face-à-face (nom Steam + race du deck de chaque camp, voir `NetLobby._show_vs_screen`) s'affiche avant que les deux clients ne basculent sur `Battle.tscn` ensemble ; `NetContext` (statique) transporte le `NetworkManager` et le résultat du handshake à travers le changement de scène.
