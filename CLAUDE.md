...
- ⬜ À faire : build Steam, étendre le prototype Arena au réseau à 8 joueurs, animations shaders, tests automatisés (unitaires GUT existants — étendre la couverture), routes backend du matchmaking classé (client déjà prêt côté `card-game`, contrat dans `docs/backend-contracts/ranked-matchmaking-and-retention.md` — à implémenter côté `wyrdane-backend`), saisons ranked avec récompenses de fin de saison, quêtes hebdomadaires et parrainage (client déjà prêt côté `card-game`, contrat dans `docs/backend-contracts/weekly-quests-and-referral.md` — à implémenter côté `wyrdane-backend`), bouton « Ouvrir un pack gratuit » dans `PackShop.tscn` (solde `CurrencyManager.free_packs` déjà câblé, pas encore exposé dans la boutique)
...
- **Quêtes hebdomadaires** et **parrainage** : squelette client écrit contre `docs/backend-contracts/weekly-quests-and-referral.md`, routes backend pas encore implémentées (échec silencieux/propre en attendant, même philosophie que le matchmaking classé avant ses routes). Quêtes hebdo : `QuestsPanel._populate_weekly` ajoute une section sous les quêtes quotidiennes dans la même vue (`InfoView.QUESTS`), récompense en packs (`reward_pack`, pas d'or) créditée sur `CurrencyManager.free_packs` (nouveau solde séparé de `balance`, synchronisé par `sync_from_backend`, consommé par `CurrencyManager.open_owned_pack`). Parrainage : `ReferralPanel.gd` (même pattern statique que `QuestsPanel`/`ProfilePanel`) ajoute une section dynamique en bas de la vue Profil (pas de nouvel écran/bouton de nav) avec le code du joueur, un bouton « copier le lien », le statut du parrainage en cours et un champ pour entrer un code reçu. Un joueur ne peut parrainer qu'**un seul** ami (contrainte prévue côté serveur, `referrer_id` unique) — récompense (3 packs + 500 or) créditée au parrain quand le filleul termine le tutoriel, pas de nouvelle route dédiée pour ce déclenchement (réutilise `POST /api/collection/claim-starter` existant).
