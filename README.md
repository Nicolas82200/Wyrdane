...
- **Choix du Pacte** (`PactChoiceSystem` + `CardEffect.pact_bonus`) : chaque effet d'une carte PACTE X est marqué "de base" (toujours exécuté, gratuitement) ou "bonus" (`pact_bonus = true`, ne s'exécute que si le joueur paie X PV). `EffectManager.trigger_effects` sépare les deux listes à chaque déclenchement du trigger concerné (Arrivée comprise pour un serviteur — aucun traitement à part, ni choix fait en amont à la pose de la carte) : les effets de base s'exécutent toujours ; s'il existe un bonus pour ce trigger, `PactChoiceSystem.resolve_trigger` est appelé (popup `ask` pour le propriétaire réel de la carte, en solo comme en réseau — plus d'heuristique pour le joueur local, réservée à l'IA seule) ; payé, le coût est appliqué (`HeroSystem.self_damage`) et les effets bonus s'exécutent en plus des effets de base, ou à leur place si `pact_replaces_base` est vrai sur le bonus (ex: invoque un serviteur amélioré au lieu du serviteur de base). Même logique côté Rituel/Enchantement dans `TriggerSystem._fire_on_enchantments`/`activate_sacrifice_ritual`/`try_cancel_spell` — la charge d'un Rituel est consommée que le bonus soit payé ou non.
  - **Synchronisation réseau** (`PactChoiceSystem`, commandes `PACT_REQUEST`/`PACT_CHOICE`/`PACT_ANNOUNCE` de `NetCommand`) : le propriétaire de la carte décide toujours lui-même, des deux côtés de la connexion. Cas courant (son propre déclencheur, ex. Arrivée d'un de ses serviteurs) : il annonce d'abord au pair qu'il commence à décider (`PACT_ANNOUNCE`, envoyé AVANT d'ouvrir sa propre popup de choix) puis notifie sa décision (`PACT_CHOICE`) une fois choisie — les deux avant même d'émettre la commande de l'action elle-même (`NetEmitter` n'émet qu'une fois la résolution locale terminée). Sans cette annonce préalable, le pair ne saurait rien de la carte tant que cette commande n'arrive pas — à ce moment-là la décision serait déjà connue (déjà reçue via `PACT_CHOICE`), et la carte semblerait n'avoir jamais été "en attente" (voir le cas de Croc de Braise/Embermaw, qui combine ciblage et Pacte au moment même où le serviteur est posé) ; l'annonce déclenche donc côté pair l'affichage immédiat de la carte + un message d'attente (même popup que le cas croisé ci-dessous), dont la réponse est mise en cache pour que le rejeu ultérieur de l'action (déclenché par `PLAY_CARD`) la consomme directement sans réafficher de popup. Cas croisé (l'action EN COURS d'un camp touche une carte Pacte du camp ADVERSE, ex. Blessure causée par une attaque sur son serviteur) : le camp qui résout en direct ne peut pas attendre passivement une décision qui n'existe pas encore côté pair — il envoie une requête explicite (`PACT_REQUEST`) et bloque sa résolution en attendant la réponse ; le propriétaire répond hors-tour dès réception (popup affichée même si ce n'est pas son tour), et met sa décision en cache pour ne pas être redemandé quand cette même action est rejouée normalement chez lui peu après.
...
Race neutre (`Race.Type.ARTIFACT`, thème « Reliques Anciennes ») : aucune identité de mots-clés exclusifs, aucun pool de mana dédié, mélangeable librement dans n'importe quel deck (`Race.is_raceless()` l'exclut, comme `Race.Type.NONE`, de l'identité de race d'un deck/du coût de race/des quêtes par race — mais avec sa propre couleur de fond/icône dans `Card.gd`, contrairement à `NONE`). Deux systèmes moteur génériques ajoutés pour elle (réutilisables par n'importe quelle race future) :
...
- **Réactivation de tous les déclencheurs** (effet `RetriggerAllTriggers`) : pour la cible résolue, rejoue l'intégralité de `target.card_data.get_trigger_names()` via `EffectManager.trigger_effects` — une réactivation ponctuelle déclenchée par un sort ou un trigger de Rituel, qui couvre TOUS les triggers de la cible (contrairement à l'Écho de trigger, passif et limité à un seul type choisi à l'avance). `EffectManager._retrigger_all`. Porté par Écho du Premier Geste, Rappel du Grand Rite, L'Éveilleur de Strates, Rituel du Second Geste, Rituel des Strates Sans Fin.
- **Dégâts de zone récurrents** (effet `DamageAllMinionsRecurring`) : variante en boucle de `DamageAllMinions` — après chaque vague de dégâts, continue seulement si au moins un serviteur du camp adverse à la source est mort à cette vague précisément, plafonnée défensivement à 20 vagues. `EffectManager._damage_all_minions_recurring`. Porté par Onde du Cataclysme.
- **Ciblage joueur généralisé sur un trigger de Rituel/Enchantement** : `EffectManager.resolve_trigger_target` (déjà utilisé pour le Dernier Souffle d'un serviteur) est aussi appelé par `TriggerSystem._execute_enchantment_effects_with_proxy` quand un trigger de Rituel/Enchantement n'a pas de cible contextuelle utilisable — le joueur local choisit alors une cible, sinon tirage aléatoire (IA/adversaire réseau). Générique sur n'importe quel trigger (Éveil, Carnage, Deuil...). Voir `CARDS.md` → section Artefact → « Points d'intégration tranchés » pour le détail du cas particulier du Deuil.

Pierre Volcanique (jeton) peut cibler soit un serviteur ennemi, soit le héros ennemi directement (`CardEffect.target = "EnemyHeroOrMinion"`, voir `CARDS.md` → section Artefact → « Points d'intégration tranchés »).

### 🧝 Elfe & 🪓 Nain — proposition de design (brouillon, NON validé)

**⚠️ Ceci est un premier jet à discuter, pas une spec figée.** Contrairement aux cinq races ci-dessus (dont les mots-clés sont validés et implémentés), rien ici n'est codé et rien ne doit être codé avant validation explicite — conformément à la règle du projet (« Notes pour les agents » dans `CLAUDE.md`) qui interdit d'inventer un mot-clé sans qu'il soit d'abord acté dans ce document. Objectif de ce brouillon : donner à chaque race une identité mécanique nette, distincte des races existantes (Mort-Vivant = attrition/poison, Humain = tempo/leadership, Démon = risque/paiement en PV, Abomination = croissance aléatoire/absorption, Artefact = écho/duplication), en cohérence avec le ton dark fantasy du jeu (pas de fantasy lumineuse classique).
...
**Réseau uniquement** — un tour où le joueur n'a plus **aucune action possible** (voir le halo doré existant du bouton Fin du tour, `Battle._player_has_no_actions`, affiché lui dans les deux modes) affiche/resserre immédiatement ce timer à 10s, pour nudger vers la fin de tour sans attendre le seuil d'inactivité normal. En solo, le halo doré suffit : pas de compte à rebours forcé sur une simple main sans jouable, pour ne jamais afficher le timer avant le seuil normal de 30s.
...
*   **Toute l'UI est traduite** (menus, deck builder, bataille, cimetière, chargement) ainsi que **les 392 cartes** (jetons compris ; noms, effets, flavour).
...
*   Cinq races jouables : Mort-Vivant, Humain, Démon, Abomination et Artefact (race neutre, `Race.Type.ARTIFACT`, sans pool de mana dédié) (392 cartes au total, jetons compris, voir `CARDS.md`) — mots-clés propres à chaque race (`KeywordUndead.gd`, `KeywordHuman.gd`, `KeywordDemon.gd`, `KeywordAbomination.gd`), mécaniques Démon (Corruption, dégâts auto-infligés `HeroSystem.self_damage`, trigger `OnSelfDamage`), Abomination (Mutation, trigger `OnDevoration`) et Artefact (écho de trigger `CardData.echoed_trigger`, mimétisme complet `MimicMinion`, ajout de carte en main `AddCardToHand`, réactivation de tous les déclencheurs `RetriggerAllTriggers`, dégâts de zone récurrents `DamageAllMinionsRecurring`)
...
*   **Internationalisation FR/EN** — toute l'UI et les 392 cartes (jetons compris), via le système de traduction natif Godot (`translations/game.csv`)
...
| Artefact | *(aucune)* | Race neutre (`Race.Type.ARTIFACT`) : pas de pool dédié — `CostSystem.get_race_cost` renvoie 0, tout le coût est `generic_cost`, payable depuis n'importe quel pool en surplus. |
