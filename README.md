...
- **Trigger `OnPactPaid`** (« Pacte payé ») : déclenché par `PactChoiceSystem.resolve_trigger` dès qu'un Pacte est effectivement payé (n'importe laquelle des cartes du joueur, pas seulement celle du rituel/enchantement qui y réagit) — càd sur les enchantements/rituels du camp qui vient de payer (`TriggerSystem.fire`, garde-fou anti-boucle `PactChoiceSystem._firing_pact_paid` si le rituel qui réagit porte lui-même un Pacte payé). Utilisé par Communion Infernale (« Quand un Pacte est payé, piochez une carte »).
...
*   `OnPactPaid` (Pacte payé — le joueur vient de payer le coût en PV d'un Pacte, sur n'importe laquelle de ses cartes)
