...
- **Choix du Pacte** (`PactChoiceSystem` + `CardEffect.pact_bonus`) : chaque effet d'une carte PACTE X est marqué "de base" (toujours exécuté, gratuitement) ou "bonus" (`pact_bonus = true`, ne s'exécute que si le joueur paie X PV). `EffectManager.trigger_effects` sépare les deux listes à chaque déclenchement du trigger concerné (Arrivée comprise pour un serviteur — aucun traitement à part, ni choix fait en amont à la pose de la carte, **sauf carte "standalone", voir juste en dessous**) : les effets de base s'exécutent toujours ; s'il existe un bonus pour ce trigger, `PactChoiceSystem.resolve_trigger` est appelé (popup pour le joueur local hors réseau, heuristique de marge de PV pour l'IA et pour les deux camps en partie réseau — `heuristic_decision`, aucun canal réseau dédié à ce choix répété) ; payé, le coût est appliqué (`HeroSystem.self_damage`) et les effets bonus s'exécutent en plus des effets de base, ou à leur place si `pact_replaces_base` est vrai sur le bonus (ex: invoque un serviteur amélioré au lieu du serviteur de base). Même logique côté Rituel/Enchantement dans `TriggerSystem._fire_on_enchantments`/`activate_sacrifice_ritual`/`try_cancel_spell` — la charge d'un Rituel est consommée que le bonus soit payé ou non.
- **Pacte "standalone"** (`CardData.pact_standalone` + `PactActivationSystem`) : pour une carte dont le Pacte est le SEUL effet (aucun effet de base sur son trigger, ex. Larve Infernale "Pacte 1 : +1/+1 permanent", rien d'autre) — décider à l'Arrivée forcerait un choix définitif dès la pose, sans marge de PV connue à l'avance. `EffectManager.trigger_effects` ignore alors complètement ce bonus à son trigger (ni proposé, ni exécuté) ; à la place, un bouton "P" reste affiché sur le serviteur en jeu tant que son propriétaire ne l'a pas activé (`Minion.pact_activated`, une seule fois), utilisable à n'importe quel moment de son propre tour — même popup Oui/Non que le choix normal (`PactChoiceSystem.ask`). Portée limitée aux effets ciblant `Self` (buff/mot-clé permanent) ; les cartes PACTE à cible ennemie (Croc de Braise, Le Grand Pacte) gardent le choix classique à l'Arrivée. Réseau : nouvelle commande `ACTIVATE_PACT` (net_id du serviteur uniquement — toujours "oui" côté rejeu, seul celui qui clique décide).
