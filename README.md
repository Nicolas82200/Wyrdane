...
|---|---|---|
| `PACTE X` | À l'arrivée en jeu, le joueur choisit de payer X PV (une valeur propre à la carte, indépendante de son coût en mana) pour activer l'effet d'Arrivée de la carte ; sans paiement, la carte se pose sans effet. | `KeywordChoiceDemon.value` (X), choix et coût en HP dans `CardSystem.handle_card_played` / `BoardSystem.summon_minion_return`, popup `PactChoiceSystem` |
...
- **Choix du Pacte** (`PactChoiceSystem` + `KeywordChoiceDemon.value`) : à la pose d'un serviteur PACTE X depuis la main, une popup de confirmation demande au joueur local s'il paie X PV. Accepté : le coût en PV est appliqué (`HeroSystem.self_damage`) puis l'effet d'Arrivée (trigger `ONPLAY`) de la carte s'exécute normalement, ciblage compris s'il y en a un. Refusé : la carte se pose comme un vanille, sans coût ni effet ni ciblage. L'IA décide seule via une heuristique de marge de PV (`AISystem._decide_pact_payment`) ; en réseau, le choix du joueur local est transporté tel quel dans `NetCommand.PLAY_CARD` (`pact_paid`) et rejoué à l'identique côté pair (`NetworkOpponent`).
