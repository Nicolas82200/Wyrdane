...
## ressource/pioche en Campagne, voir CAMPAIGN.md), hors Enchantement/Rituel
...
## CampaignRewardPicker pour la récompense de combat) et hors Incantation
## (Instant) : le plateau de Campagne est un auto-battler, seules des
## créatures peuvent y être posées (une carte à effet immédiat n'a pas
## d'emplacement de plateau propre). Hors doublon avec les cartes déjà
## proposées dans les choix précédents de cette même run (pour éviter de
## re-proposer 5 fois la même poignée de cartes vues plus tôt).
