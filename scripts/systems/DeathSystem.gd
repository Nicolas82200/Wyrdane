...
func process_deaths() -> void:
	...
	var dead_player: Array[Minion] = battle.player_minions.filter(
		func(m: Minion): return m.is_dead()
	)
	...
	# Tous les triggers de mort avec await
	await _trigger_deathrattle(dead_player)
	await _trigger_deathrattle(dead_enemy)
	await _trigger_grief(dead_player, true)
	await _trigger_grief(dead_enemy, false)
	await _trigger_death_rage(dead_player, true)
	await _trigger_death_rage(dead_enemy, false)
...
func _trigger_grief(dead_minions: Array[Minion], dead_were_player: bool) -> void:
	...
	for survivor in survivors:
		await battle.effect_manager.trigger_effects(battle, survivor, "OnGrief")

func _trigger_death_rage(dead_minions: Array[Minion], dead_were_player: bool) -> void:
	if dead_minions.is_empty():
		return
	var has_undead_death := dead_minions.any(func(m: Minion): 
		return m.card_data.race == Race.Type.UNDEAD
	)
	if not has_undead_death:
		return
	var survivors: Array[Minion] = battle.player_minions if dead_were_player else battle.enemy_minions
	for survivor in survivors:
		await battle.effect_manager.trigger_effects(battle, survivor, "OnDeathRage")
