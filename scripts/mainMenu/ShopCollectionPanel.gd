...
# stock ne distingue plus l'origine). Ouvrir un lot (1/2/5/10, borné au stock
# réel) délègue à `open_callback` (voir MainMenu._open_owned_packs_flow), qui
# affiche PackShop pour l'animation de révélation.

const OPEN_QUANTITIES := [1, 2, 5, 10]

## Construit le panneau dans `parent` (VBoxContainer, vidé puis reconstruit) :
## compteur de packs en stock puis une rangée de boutons "Ouvrir N", chacun
## désactivé si le stock est inférieur à N.
