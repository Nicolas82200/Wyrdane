...
func _restore_in_hand() -> void:
	...
	var hand_node = get_parent().get_parent() if get_parent() and get_parent().name == "CardsContainer" else get_parent()
...
func _return_to_hand() -> void:
	...
	if battle:
		...
		var cont: Control     = hand.get_node_or_null("CardsContainer") if hand else null
		if cont:
			reparent(cont)
