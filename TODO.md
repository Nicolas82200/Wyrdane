...
**Résolu après troisième retour utilisateur.** Les Incantations (`card_type == "Instant"`, sorts à effet immédiat) apparaissaient encore dans les choix de plateau (construction initiale, récompense de combat/boutique, événements) alors que `CampaignBattle._place_card_on_board()` ne sait poser qu'un Serviteur ou une Relique (Enchantement/Rituel) — une Incantation choisie y aurait été invoquée à tort comme un serviteur cassé via `board_system.summon_minion`. `CampaignBoardBuild.pick_candidates`, `CampaignRewardPicker._pick_one` et `CampaignEvents._add_random_card` filtrent désormais strictement sur `card_type == "Minion"` (au lieu d'une simple exclusion Ressource/Enchantement/Rituel) — seules des créatures peuvent rejoindre le plateau du joueur. Même correctif côté plateau adverse : `CampaignOpponentFactory._pick_card` exclut aussi les Incantations du pool généré pour l'IA.
